...
import { Search, Filter, X, Calendar, Grid, List, Clock, MapPin, Heart, Eye } from 'lucide-react';
...
const categoryColors: Record<string, string> = {
    ...
    Academic: 'bg-blue-500/20 text-blue-600',
    Sports: 'bg-green-500/20 text-green-600',
    Workshop: 'bg-orange-500/20 text-orange-600',
    Scholarship: 'bg-yellow-500/20 text-yellow-600',
    Club: 'bg-indigo-500/20 text-indigo-600',
};
                ...
                <div className="max-w-7xl mx-auto px-4 relative">
                    ...
                    >
                        ...
                        <p className="text-muted-foreground text-lg max-w-xl mx-auto">
                            Discover and register for future campus events
                        </p>
                    </motion.div>
                        ...
                        <div className="flex flex-col lg:flex-row gap-4">
                            ...
                            <div className="flex flex-wrap gap-3">
                                <Select value={selectedCategory} onValueChange={setSelectedCategory}>
                                    <SelectTrigger className="w-40 h-12 bg-background/50 border-border/50 rounded-xl">
                                ...
                                <Select value={sortBy} onValueChange={setSortBy}>
                                    <SelectTrigger className="w-36 h-12 bg-background/50 border-border/50 rounded-xl">
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <p className="text-muted-foreground">
                            Showing <span className="font-semibold text-foreground">{sortedEvents.length}</span> upcoming events
                        </p>
                    </div>
                    ...
                    {isLoading ? (
                        viewMode === 'grid' ? (
                            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {[...Array(6)].map((_, i) => (
                                    <div key={i} className="glass rounded-2xl p-4 flex flex-col gap-4 animate-pulse h-[400px]">
                                        ...
                                        return (
                                            ...
                                            >
                                                <Link href={`/event/${event.id}`}>
                                                    <div className="glass rounded-xl p-4 hover:bg-primary/5 transition-all cursor-pointer group">
                                                        <div className="flex items-center gap-4">
                                                            {/* Thumbnail */}
                                                            <img
                                                                src={event.bannerUrl}
                                                            ...
                                                            <Badge
                                                                variant="secondary"
                                                                className={`hidden sm:inline-flex ${categoryColors[event.category] || 'bg-gray-500/20 text-gray-600'}`}
                                                            >
                                                                {event.category}
                                                            </Badge>
                        ...
                        >
                            ...
                            <h3 className="font-display text-2xl font-bold mb-2">No upcoming events found</h3>
